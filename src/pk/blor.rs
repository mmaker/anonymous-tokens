...
fn test_sound() {
    ...
    let signing_key = SigningKey::new(csrng);
    let verification_key = VerifierKey::from(&signing_key);
    // this is a token generated from another key, and it should lead to
    // - incorrect verification
    // - incorrect read
    let token = bincode::deserialize::<Token>(&[
        4, 119, 140, 240, 231, 157, 68, 49, 1, 180, 76, 204, 142, 236, 151, 29, 100, 68, 142, 38,
        54, 127, 231, 235, 63, 190, 47, 11, 214, 51, 165, 118, 34, 213, 61, 7, 221, 78, 49, 255,
        126, 249, 18, 16, 48, 64, 60, 151, 255, 188, 136, 246, 235, 7, 7, 246, 130, 116, 182, 14,
        60, 97, 82, 14, 141, 192, 231, 125, 160, 79, 145, 94, 182, 116, 72, 8, 211, 159, 112, 173,
        234, 155, 156, 220, 104, 63, 64, 124, 149, 189, 47, 9, 234, 109, 84, 4, 210, 78, 62, 98,
        131, 202, 116, 124, 163, 17, 173, 63, 38, 185, 183, 62, 178, 90, 53, 14, 17, 10, 108, 199,
        17, 1, 33, 1, 74, 104, 29, 15, 156, 210, 140, 142, 7, 73, 230, 22, 203, 114, 161, 177, 224,
        176, 234, 226, 50, 216, 83, 232, 92, 191, 52, 40, 23, 107, 162, 37, 96, 95, 11, 10, 150,
        244, 224, 132, 177, 187, 254, 84, 219, 214, 203, 135, 117, 221, 170, 0, 189, 202, 67, 217,
        0, 238, 100, 122, 241, 101, 224, 201, 213, 91, 190, 56, 166, 75, 155, 0, 158, 27, 216, 54,
        14, 198, 136, 2, 221, 233, 25, 56, 162, 93, 210, 181, 183, 36, 112, 231, 174, 95, 155, 60,
        17, 201, 43, 9, 48, 250, 115, 178, 181, 236, 208, 214, 209, 217, 29, 195, 149, 163, 49,
        139, 153, 243, 147, 212, 228, 5, 175, 9, 194, 106, 41, 84, 193, 127, 20, 50,
    ]);
    assert!(token.is_ok());
    let token = token.unwrap();

    assert!(verification_key.verify(&token).is_err());
    assert!(signing_key.read(&token).is_err());
}
