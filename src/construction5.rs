...
impl TokenBlinded {
    pub fn unblind(self, ts: TokenSigned) -> Result<Token, zkp::ProofError> {
        let S0 = self.secret.blind[0][0].invert()
            * RistrettoPoint::hash_from_bytes::<Sha512>(&ts.s)
            + self.secret.blind[0][1] * self.pp.gen[1];
        let S1 = self.secret.blind[1][0].invert()
            * RistrettoPoint::hash_from_bytes::<Sha512>(&ts.s)
    ...
    pub fn to_bytes(&self) -> [[u8; 32]; 2] {
        return [
            self.public[0].compress().to_bytes(),
...
mod tests {
    ...
    fn crashes() {
        ...
        let gen = RISTRETTO_BASEPOINT_POINT;
        let pk = sk * gen;

        let mut transcript = Transcript::new(b"blabla");
        ...
        let another_proof: zkp::BatchableProof = unsafe { std::mem::transmute_copy(&proof) };
    }
}
