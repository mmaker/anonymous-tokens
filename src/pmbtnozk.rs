...
impl TokenBlinded {
    pub fn unsafe_unblind(&self, ts: &TokenSigned) -> Result<Token, zkp::ProofError> {
        ...
        let S = [
            self.secret.multiplicative_blind[0] * RistrettoPoint::hash_from_bytes::<Sha512>(&ts.s) + self.secret.additive_blind[0] * self.pp.gen[1],
            self.secret.multiplicative_blind[1] * RistrettoPoint::hash_from_bytes::<Sha512>(&ts.s) + self.secret.additive_blind[1] * self.pp.gen[1],
        ];
        let signature = [
            self.secret.multiplicative_blind[0] * ts.signature + self.secret.additive_blind[0] * self.pp.pk[0],
            self.secret.multiplicative_blind[1] * ts.signature + self.secret.additive_blind[1] * self.pp.pk[1],
        ];

        Ok(Token { ticket, S, signature })
    }
...
impl TokenSecret {
    ...
    where R: RngCore + CryptoRng,
    {
        ...
        Self { ticket, additive_blind, multiplicative_blind }
    }
}
...
impl PublicParams {
        ...
        R: RngCore + CryptoRng,
    {
        ...
        let public = [
            secret.multiplicative_blind[0].invert() * (hashed_ticket - secret.additive_blind[0] * self.gen[0]),
            secret.multiplicative_blind[1].invert() * (hashed_ticket - secret.additive_blind[1] * self.gen[0]),
        ];
